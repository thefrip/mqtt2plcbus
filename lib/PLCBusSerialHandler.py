...
class serialHandler(threading.Thread):
    ...
    def __init__(self, serial_port_no, command_cb, message_cb, log):
        ...
        # logger
        self.log = log
        self.log.debug("Starting PLCBusSerialHandler")
        ...
        #serial port init
        self.__myser = serial.Serial(port = serial_port_no, baudrate = 9600, timeout = 0.4,
...
#        self._mutex = mutex.mutex()
    ...
    def _send(self, plcbus_frame):
        ...
        self.log.debug("Sending PLC bus code %s " % plcbus_frame)
        if (int(plcbus_frame[8:10], 16) >> 5) & 1: #ACK pulse bit set to 1
         ...
         #   time.sleep(0.3)
            ...
            while not self._stop.isSet():
                ...
                self.log.debug("Sending frame with ACK")
                self.needs_ack_for(plcbus_frame)
                self._basic_write(plcbus_frame)
                time.sleep(0.6)
                self.log.debug("time before wait : %s" % time.time())
                for i in range(3):
                    self.receive()
                    if self._ack.isSet():
                        print("got ack in first read")
                        break
                self.log.debug("time after wait : %s" % time.time())
                if self._ack.isSet():
                ...
                if (ACK_received == 1):
                    ...
                    self.log.debug("WARN : Message %s sent, but ack never received" % plcbus_frame)
                    break #2s
            ...
            self.log.debug("Sending frame without ACK")
            self._basic_write(plcbus_frame)
            time.sleep(2.5)
            self._cb(explicit_frame)
        else:
            self.log.debug("Sending GET_ALL frame without ACK")
            self._basic_write(plcbus_frame)
            time.sleep(2.5)
...
#        try:
        ...
        if self.__myser.inWaiting() < 9:
            time.sleep(0.4) # Wait for max 400ms if nothing to read
            return
        message = self.__myser.read(1) # Read header
        self.log.debug("RECEIVING byte: '%s'" % bytes.hex(message))
        if bytes.hex(message) == "02":
            message = self.__myser.read(1) # Read length (if not already read, in case message is incomplete), otherwise consider that length already in message
            self.log.debug("RECEIVING 2nd byte: %s" % bytes.hex(message))
        
        if bytes.hex(message) != "06":
            self.log.debug("WRONGLY FORMED MESSAGE")
            return

        message = self.__myser.read(7) # Read message content
#        except IOError:
#            pass
        if(message):
            m_string = "0206" + bytes.hex(message)
            self.log.debug("RECEIVED string : %s" % m_string)
            ...
            if self._is_from_myself(m_string):
                self.log.debug("FROM myself")
                return
                
            if self._is_answer(m_string):
                self.log.debug("ANSWER : %s" % m_string)
                
                #update heartbeat file
                open("/tmp/PLCBus.heartbeat", 'a').close()
                utime("/tmp/PLCBus.heartbeat", None)
                self._cb(self.explicit_message(m_string))
                self.log.debug("UPDATING plcbus heartbeat")
                
            elif self._is_ack(m_string):
                
                self.log.debug("IS ACK : %s, waited ack : %s" % (m_string, self._waited_ack))
                if (self._waited_ack != None) and self._is_ack_for_message(m_string, self._waited_ack):
                    self._waited_ack = None
                    self._ack.set()
            else:
                self.log.debug("QUEUE : %s" % m_string)
                self._cb(self.explicit_message(m_string))
                
            self.log.debug("RECEIVED done")
