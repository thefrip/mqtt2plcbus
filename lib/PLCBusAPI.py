...
from lib.PLCBusSerialHandler import serialHandler
...
class PLCBUSAPI:
    ...
    def __init__(self, log, serial_port_no, command_cb, message_cb):
        ...
        self._ser_handler = serialHandler(serial_port_no, command_cb, message_cb, log)
    ...
    def send(self, cmd, item, ucod, data1 = "00", data2 = "00"):
        ...
        try:
            ...
            self._log.warning("PLCBUS Frame generation error, command does not exist ", cmd)
        else:
            ...
            try:
                message = bytes.fromhex(plcbus_frame)
            except (TypeError, UnicodeDecodeError) as e:
                self._log.warning("PLCBUS Frame generation error, does not result in a HEX string ", plcbus_frame)
            else:
